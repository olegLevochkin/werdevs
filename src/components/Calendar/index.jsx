            ...
            <div className="calendar">
                ...
                <table>
                    ...
                    {monthData.map((week, ind) =>
                        <tr key={ind} className="week">
                            {week.map((date, index) =>
                                (index !== 0 && week[index-1] ) ?
                                <TD isOpen={isOpen} onChangeOpen={onChangeOpen} index={index-1} ind={ind}  currentDate={currentDate}  monthData={monthData} /> :
                                (index === 0 && monthData[ind-1] ) ?
                                <TD isOpen={isOpen} onChangeOpen={onChangeOpen} is={true} index={6} ind={ind-1}  currentDate={currentDate}  monthData={monthData} /> :
                                    (ind === 0 && countFirstDays !== 6 ) ?
                                    <td key={index} className="day grey" >{lastDay - countFirstDays--}</td>:
                                        (ind === monthData.length-1 ) ?
                                            <td key={index} className="day grey" >{countLastDays++}</td>: null
